...
def _plain(value: Any) -> Any:
    """Plain scalars, lists, and dicts, all the way down.

    ``ScenarioConfig`` nests a dataclass, an enum, and tuples. ``yaml.safe_dump`` refuses
    all three, and the same dict is served over the public drill API, so both consumers
    get the flattened form rather than one of them getting a ``RepresenterError``.
    """
    if isinstance(value, Enum):
        return value.value
    if is_dataclass(value) and not isinstance(value, type):
        return {key: _plain(inner) for key, inner in vars(value).items()}
...
class DrillSpec:
    ...
    def as_dict(self) -> dict[str, Any]:
        return {
            ...
            # The scenario is half of what makes a drill reproducible. Exporting the
            # faults without it published a corpus that said what was broken and never
            # said what it was broken against.
            "scenario": _plain(self.scenario),
            "faults": [
                {
                    "tool": f.tool,
...
DRILLS: list[DrillSpec] = [
    ...
    DrillSpec(
        ...
        description=(
            "The capacity effect commits and the response is lost. The retry re-plans to "
            "a different destination, which derives a different action id, so it never "
            "meets the first receipt. Instrumented across seeds, a scripted run makes 145 "
            "commit_effect calls and replays zero receipts. What refuses the second "
            "reservation is N1: one live reservation per placement group."
        ),
    ...
    DrillSpec(
        ...
        description=(
            "The same lost-response shape as D5, on the facilities effect. Instrumented "
            "across seeds, a scripted run makes 143 commit_effect calls, exactly one "
            "create_work_order call, and replays zero receipts. Nothing retries after the "
            "loss, so what this drill establishes is that the lost response leaves one "
            "work order behind. The retry path stays unexercised on the scripted tier."
        ),
        scenario=ScenarioConfig(max_rounds=6),
    ...
    DrillSpec(
        ...
        description=(
            "The custody commit is interrupted after the effect lands. Instrumented across "
            "seeds, a scripted run makes 143 commit_effect calls, exactly one "
            "commit_ready_transfers call, and replays zero receipts. Every container still "
            "commits once, and the run does not resume after the loss, so no receipt "
            "replay is exercised on the scripted tier."
        ),
...
def main() -> int:  # pragma: no cover - CLI
    ...
    ``export_corpus`` had no caller and no Make target, so the YAML under ``corpus/`` was
    only ever regenerated by hand and could silently fall behind the specs in this file.
    """
    import argparse

    parser = argparse.ArgumentParser(description="Export the drill corpus as YAML.")
    ...
    export_corpus(root)
    print(
        f"wrote {len(DRILLS)} public and {len(HOLDOUT_DRILLS)} holdout drill(s) "
        f"to {root}/ at corpus version {CORPUS_VERSION}"
    )
    return 0
